...
import { ArrowRight, X, Play, ShoppingCart } from 'lucide-react';
      ...
      <main className="flex flex-col items-start justify-center min-h-screen container mx-auto px-6 pt-20">
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            className="px-8 py-4 rounded-xl text-lg font-bold text-white border border-white/10 hover:bg-white/5 transition-colors text-center flex items-center gap-2"
          >
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-200">
          <div className="relative w-full max-w-5xl aspect-video bg-black rounded-2xl overflow-hidden shadow-2xl border border-white/10">
            <button
              ...
              className="absolute top-4 right-4 z-10 bg-black/50 hover:bg-black/80 text-white p-2 rounded-full backdrop-blur-sm transition-colors"
            >
              <X size={24} />
            </button>
            <video
      ...
      <section className="py-24 bg-neutral-900/30 backdrop-blur-sm border-t border-white/5">
        <div className="container mx-auto px-6 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {[
              { name: "AP Abshan", image: "/team/abshan.jpg" },
              { name: "Alen Eldho Jobi", image: null },
              { name: "Viswajith A", image: null },
              { name: "Amith S", image: "/team/amith.jpg" }
            ].map((member) => (
              <div key={member.name} className="group p-6 rounded-xl bg-white/5 border border-white/10 flex flex-col items-center hover:bg-white/10 hover:border-blue-500/50 hover:shadow-[0_0_30px_rgba(59,130,246,0.15)] transition-all duration-300">
                <div className="w-20 h-20 bg-neutral-800 rounded-full mb-4 flex items-center justify-center text-xl font-bold text-neutral-400 overflow-hidden relative group-hover:scale-110 group-hover:shadow-[0_0_20px_rgba(59,130,246,0.5)] transition-transform duration-300">
                  {member.image ? (
